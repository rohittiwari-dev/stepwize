...
export const LightPillar: React.FC<LightPillarProps> = ({
			...
			renderer = new THREE.WebGLRenderer({
				...
				powerPreference:
					effectiveQuality === 'high'
						...
						: 'low-power',
				precision:
					settings.precision as THREE.WebGLRendererParameters['precision'],
				stencil: false,
				depth: false,
			});
		} catch (error) {
    ...
    `;
		...
		const resizeObserver = new ResizeObserver((entries) => {
			for (const entry of entries) {
				...
				const newWidth = entry.contentRect.width || 1;
	...
	if (!webGLSupported) {
		return (
			<div
				className={`absolute inset-0 flex items-center justify-center bg-black/10 text-muted-foreground text-sm ${className}`}
			>
				WebGL not supported
			</div>
		);
